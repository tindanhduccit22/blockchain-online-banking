...
# 8.3 Creative Challenges (Bonus Features)

This project implements two major Creative Challenges to enhance protocol safety and financial resilience:

## 8.3.1 Challenge C1: Principal Safety Guarantee (Deferred Interest Claim)

### Problem in Base Spec
In the base specification, `withdrawAtMaturity` reverts if the vault lacks sufficient funds to pay interest. This creates a severe liquidity lock risk for the user: the bank could lock the user's principal indefinitely simply by keeping the interest vault unfunded.

### Implemented Solution
We implemented `withdrawPrincipalOnlyAtMaturity(uint256 depositId)` and `claimPendingInterest(uint256 depositId)`.
- At maturity, even if the interest vault balance is 0, the user can call `withdrawPrincipalOnlyAtMaturity()` to immediately retrieve 100% of their deposited principal from `SavingCore`.
- The owed interest is safely recorded on-chain in `pendingInterest[depositId]`.
- When the admin later funds the vault, the user can call `claimPendingInterest(depositId)` to claim their accumulated interest.

### Trade-off
Splitting principal and interest payouts requires an extra step for the user if the vault is temporarily underfunded, but it guarantees that user principal is never held hostage by an empty interest vault.

### Test Coverage
Verified by unit test:
`"should allow C1 principal-only withdrawal when vault is empty and claim interest later"`

---

## 8.3.2 Challenge C2: Vault Solvency Guard

### Problem in Base Spec
In the base specification, the admin can call `withdrawVault(amount)` to drain the entire vault balance at any time. Active deposits that expected interest payouts could suddenly be left unpayable.

### Implemented Solution
We added an on-chain Solvency Guard mechanism:
- `SavingCore` tracks `totalReservedInterest` across all active deposits and synchronizes this state with `VaultManager.updateReservedInterest(uint256)`.
- In `VaultManager.withdrawVault(uint256 amount)`, the contract enforces:
  ```solidity
  require(
      token.balanceOf(address(this)) - amount >= reservedInterest,
      "Solvency guard: cannot withdraw reserved interest"
  );
  ```
- This prevents the admin from withdrawing any funds that have already been promised to active depositors.

### Trade-off
The admin loses the ability to arbitrarily withdraw funds from the interest pool if active deposits exist, but user interest promises are strictly secured on-chain.

### Test Coverage
Verified by unit test:
`"should enforce solvency guard blocking admin from withdrawing promised interest reserve (Creative Challenge C2)"`
