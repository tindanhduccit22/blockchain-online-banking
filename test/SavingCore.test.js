...
describe("SavingCore", function () {
  ...
  describe("Creative Challenge C1 & Additional Features", function () {
    it("should allow autoRenewDeposit alias to process auto renew", async function () {
      ...
      await savingCore.connect(user).setAutoRenew(0, true);

      // Fast forward past maturity + grace period (90 + 2 = 92 days)
      await ethers.provider.send("evm_increaseTime", [92 * 86400 + 10]);
      await ethers.provider.send("evm_mine");

      // Fund vault for interest
      ...
      const deposit = await savingCore.getDeposit(0);
      expect(deposit.principal).to.be.gt(amount);
    });

    it("should allow C1 principal-only withdrawal when vault is empty and claim interest later", async function () {
      await savingCore.createPlan(90, 200, 0, 0, 400);
      const amount = ethers.parseUnits("500", 6);
      await mockUSDC.mint(user.address, amount);
      await mockUSDC.connect(user).approve(await savingCore.getAddress(), amount);
      ...
      // Fast forward past 90 days maturity
      await ethers.provider.send("evm_increaseTime", [90 * 86400 + 10]);
      await ethers.provider.send("evm_mine");

      // Vault is empty. Normal withdrawAtMaturity reverts due to insufficient vault balance
      await expect(savingCore.connect(user).withdrawAtMaturity(0)).to.be.revertedWith("Insufficient vault balance");

      // C1: User calls withdrawPrincipalOnlyAtMaturity to recover principal immediately
      const balanceBefore = await mockUSDC.balanceOf(user.address);
      await savingCore.connect(user).withdrawPrincipalOnlyAtMaturity(0);
      const balanceAfter = await mockUSDC.balanceOf(user.address);

      expect(balanceAfter - balanceBefore).to.equal(amount);
      const pendingInterest = await savingCore.pendingInterest(0);
      expect(pendingInterest).to.be.gt(0);

      // User tries to claim pending interest before funding -> reverts
      await expect(savingCore.connect(user).claimPendingInterest(0)).to.be.revertedWith("Insufficient vault balance");
      ...
      // User claims pending interest
      await savingCore.connect(user).claimPendingInterest(0);
      expect(await savingCore.pendingInterest(0)).to.equal(0);
    });

    it("should enforce min and max deposit limits on renewDeposit", async function () {
      // Create Plan 0 (unrestricted) and Plan 1 (min 1000 USDC)
      await savingCore.createPlan(90, 200, 0, 0, 400);
      await savingCore.createPlan(90, 200, ethers.parseUnits("1000", 6), ethers.parseUnits("2000", 6), 400);
      ...
      // Renew into Plan 1 with 500 USDC (< min 1000 USDC) -> reverts
      await expect(savingCore.connect(user).renewDeposit(0, 1)).to.be.revertedWith("Below minimum deposit");

      // Open a large deposit of 3000 USDC into Plan 0
      const largeAmount = ethers.parseUnits("3000", 6);
      await mockUSDC.mint(user.address, largeAmount);
      await mockUSDC.connect(user).approve(await savingCore.getAddress(), largeAmount);
      await savingCore.connect(user).openDeposit(0, largeAmount);
