...
describe("VaultManager", function () {
  ...
  it("should allow authorized SavingCore to payout interest", async function () {
    ...
    describe("VaultManager Edge Cases", function () {
      ...
      it("should enforce solvency guard blocking admin from withdrawing promised interest reserve (Creative Challenge C2)", async function () {
        const { token, vault, owner, user } = await deployVaultManager();
        const fundAmount = ethers.parseUnits("1000", 6);
        const reservedInterest = ethers.parseUnits("800", 6);

        await token.mint(owner.address, fundAmount);
        await token.approve(await vault.getAddress(), fundAmount);
        await vault.fundVault(fundAmount);

        // Set user as SavingCore and update reserved interest
        await vault.setSavingCore(user.address);
        await vault.connect(user).updateReservedInterest(reservedInterest);

        // Owner tries to withdraw 300 USDC (leaving 700 < 800 reserved) -> reverts
        await expect(vault.withdrawVault(ethers.parseUnits("300", 6))).to.be.revertedWith(
          "Solvency guard: cannot withdraw reserved interest"
        );

        // Owner withdraws 200 USDC (leaving 800 == 800 reserved) -> succeeds
        await vault.withdrawVault(ethers.parseUnits("200", 6));
        expect(await vault.getVaultBalance()).to.equal(ethers.parseUnits("800", 6));
      });
    });
  });
