...
contract SavingCore is ERC721, Ownable, Pausable {
    ...
    function openDeposit(
        ...
        uint256 expectedInterest = calculateInterest(amount, plan.aprBps, plan.tenorDays);
        totalReservedInterest += expectedInterest;
        try vaultManager.updateReservedInterest(totalReservedInterest) {} catch {}
    ...
    function processAutoRenew(uint256 depositId) public whenNotPaused {
        require(depositId < nextDepositId, "Deposit does not exist");
        ...
        uint256 newExpectedInterest = calculateInterest(newPrincipal, deposit.aprBpsAtOpen, deposit.tenorDays);
        totalReservedInterest += newExpectedInterest;
        try vaultManager.updateReservedInterest(totalReservedInterest) {} catch {}
    ...
    function renewDeposit(
        ...
        uint256 oldExpectedInterest = calculateInterest(oldPrincipal, oldDeposit.aprBpsAtOpen, oldDeposit.tenorDays);
        uint256 newExpectedInterest = calculateInterest(newPrincipal, newPlan.aprBps, newPlan.tenorDays);
        if (totalReservedInterest >= oldExpectedInterest) {
            totalReservedInterest -= oldExpectedInterest;
        } else {
    ...
    function withdrawAtMaturity(uint256 depositId) external whenNotPaused {
        ...
        if (totalReservedInterest >= interest) {
    ...
    function withdrawPrincipalOnlyAtMaturity(uint256 depositId) external whenNotPaused {
        require(depositId < nextDepositId, "Deposit does not exist");
        Deposit storage deposit = deposits[depositId];
        require(deposit.status == DepositStatus.ACTIVE, "Deposit is not active");
        require(ownerOf(depositId) == msg.sender, "Not deposit owner");
        require(block.timestamp >= deposit.maturityAt, "Deposit has not matured");

        uint256 interest = calculateInterest(deposit.principal, deposit.aprBpsAtOpen, deposit.tenorDays);
        uint256 principal = deposit.principal;
        ...
        if (interest > 0) {
            ...
            emit PendingInterestRecorded(depositId, msg.sender, interest);
        }

        emit DepositWithdrawn(depositId, msg.sender, principal, 0);
        emit Withdrawn(depositId, msg.sender, principal, 0, false);
    }

    function claimPendingInterest(uint256 depositId) external whenNotPaused {
        require(depositId < nextDepositId, "Deposit does not exist");
        require(ownerOf(depositId) == msg.sender, "Not deposit owner");
        uint256 amount = pendingInterest[depositId];
        require(amount > 0, "No pending interest");

        pendingInterest[depositId] = 0;
        vaultManager.payoutInterest(msg.sender, amount);

        emit PendingInterestClaimed(depositId, msg.sender, amount);
    }
    ...
    function withdrawEarly(uint256 depositId) external whenNotPaused {
        ...
        uint256 expectedInterest = calculateInterest(principal, deposit.aprBpsAtOpen, deposit.tenorDays);
        if (totalReservedInterest >= expectedInterest) {
            totalReservedInterest -= expectedInterest;
        } else {
            totalReservedInterest = 0;
        }
